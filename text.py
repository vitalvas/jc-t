#!/usr/bin/env python
# -*- coding: utf-8 -*-

is_txt={
    'кто здесь?'.decode('UTF-8'): 'Это я. Капитан Джек Воробей!',
    'темы!'.decode('UTF-8'): 'Темы читать сюда однако http://news.radio-t.com',
    'допилить!'.decode('UTF-8'): 'Допилить бот можно здесь http://goo.gl/cknpQ',
    'Расскажите историю про программиста'.decode('UTF-8'): 'Тайна сия великая есмь. Срыв покровов тут: http://lurkmore.to/Умпутун',
    'говори!'.decode('UTF-8'): [
        'Мои руки чисты… Фигурально выражаясь',
        'Ну и слава Богу! Нормальный не пошёл бы на такое!',
        'Я намерен реквизировать корабль, набрать людей на Тортуге, чтобы вершить разбой на потребу своей чёрной душе.',
        'Перестаньте дырявить мой корабль!',
        'Мне всё понятно!.. Кроме парика…',
        'Смекаешь?',
        'Зараза…',
        'Стойте! Я обронил мозги.',
        'Мне не пойдут щупальца! Но бессмертие, оно того стоит, верно?',
        'Ну разве ж это честно? Я про родную тётку сознался, а вы мне всякую мелочевку подсовываете. Я негодую.',
        'Пусть молодёжь сама договаривается. Чай, уже с паспортами.',
        'Князь, зачем Вам это? Я же Вас попутаю до смерти. Да и на что мы будем кушать и Вам пилюльки покупать?',
        '… ты ему только порнографию не носи, а то у него кардиограмма от нее плохая…',
        'Со свадебным путешествием поаккуратнее. Я тут недавно видел сверхсекретные карты Генштаба — там нет Америки.',
        'Нет, спасибо, меня и так прёт. Наяву. Без всякого компота.',
        'О! А здесь работницы чулочной фабрики милиционера изнасиловали.',
        'Поехали. Только за виски заплати, а то у меня платиновая American Express, не везде принимают.',
        'А еще я видел, как в Китае одного писателя казнили. Он потом в Интернете свои ощущения опубликовал.',
        'Какой вы искренний человек… как пожарный.',
        'А настоящая водка — это не пьянство, а ключ к своей совести, с нее-то и начинается настоящая мудрость.',
        'Наркотикам нет! Что такое наркотики? Разве это удовольствие? Хаос один. И кто их придумал? Хиппи некультурные. А водку? Сам Менделеев. Великий ученый. Отец всей химической таблицы элементов. Светоч.',
        'Свобода, милый, в том, чтобы не пришлось о чём-то сожалеть.',
        'Умей из всего извлечь пользу, а потом забыть.',
        'Ненависть — тяжелый багаж. Жизнь слишком коротка, чтобы тратить её на зло. Оно того не стоит',
        'Сделал ли ты что-нибудь, чтобы жизнь стала лучше?',
        'Если хочешь узнать, сколько у парня было девчонок, спроси и подели его ответ на 3.',
        'Мой отец учил меня, что страх есть всегда. Признав это, станешь сильнее.',
        'Свобода не даётся просто так. Её цена высока. За неё платят собственной кровью.',
        'Безумие…? Это — Спарта!',
        'Здесь вам не частная лавочка! Есть положение о молодых специалистах, и вы не имеете права…',
        'Заслуженные артисты всю жизнь поют одно и то же. И получают народных…',
        'Мир изменился. Я чувствую это в воде, чувствую в земле, ощущаю в воздухе. Многое из того, что было, ушло, и не осталось тех, кто помнит об этом.',
        'Маг не приходит поздно, Фродо Бэггинс, и рано тоже не приходит. Он появляется в то время, когда положено',
        'Знаете что такое извращение? Это дело вкуса, как с китайской едой, либо нравится, либо нет, но если ты китаец, у тебя нет выбора.',
        'Откуда мне знать, делал ли ты всё по своему желанию, или следовал какой-то дурацкой программе!',
        'Я люблю разговаривать сам с собой, но когда вы начинайте говорить без меня, то это выглядит странно.',
        'Вот ты, Антон, «светлый», а пиво пьёшь — тёмное.',
        'А одна девочка написала, что её сердце «чуть тревожит неясная девичья тоска по чему-то большому, что придёт и властно обхватит».',
        'Я знаю, что я простой человек и лишь делаю своё маленькое дело, и ничего более.',
        'Вы думаете, это демократия? Неее… Это просто везуха какая-то!',
        'Можно сказать, что его появление нарушило на минуту перемену блюд.',
        'Если вы считаете, что C++ труден, попытайтесь выучить английский',
        'Я мечтал, чтобы компьютеры стали такими же простыми в использовании, как телефоны. Недавно моя мечта сбылась — я не смог разобраться с моим новым телефоном.',
        'Модульность — фундаментальный аспект всех успешно работающих крупных систем.',
        'Функции используются для наведения порядка в хаосе алгоритмов.',
        'Если не выходить за границу «объектно-ориентированных» методов, чтобы остаться в рамках «хорошего программирования и проектирования», то в итоге обязательно получается нечто, по большей части не имеющее смысла.',
        'Ограничение возможностей языка с целью предотвращения программистских ошибок в лучшем случае опасно.',
        'Что бы нового мы ни создавали, мы должны дать людям возможность переходить от старых инструментов и идей к новым.',
        'Модульность — фундаментальный аспект всех успешно работающих крупных систем.',
        'Функции используются для наведения порядка в хаосе алгоритмов.',
        'Истина где-то там.',
        'Уверенность даёт силы, позволяющие идти вперёд.',
        'Войну можно выиграть только правильно выбрав решающую битву.',
        'Ничто не иссчезает бесследно.',
        'Сопротивляйся или стань рабом.',
        'Идёт борьба между Богом и Дьяволом за наши души. И мы проигрываем.',
        'Если вы носите костюм - это ещё не значит, что он вам впору.',
        'Не участвуй в чужих кампаниях и крестовых походах. Думай о своём интересе. Всегда.',
        'Перед тобой тело самой умной, самой подлой и самой сумасшедшей говорящей мартышки. Я его просто обожаю.',
        'Война — это фабрика, производящая опустошение, смерть и скрытность.',
        'Ошибается человек, а прощает лишь Бог.',
        'Психолог говорила, что мне надо учиться не бояться быть одной. А сама звала меня на сеансы пять раз в неделю. Так кто из нас боялся быть одной?',
        'Они могут забрать мой дом, мои деньги и мои игрушки. Но есть то, что они никогда у меня не заберут. Я — Железный человек.',
        'Никакой политики. Лишь старомодная месть!',
        'Уважаемый, был ли это просто картинный жест, или вы навели на меня оружие с целью нанести смертельные увечья?',
        'Храбрость — это терпение, терпение в опасности. Это победа. Русский солдат храбр, стоек и терпелив! Потому непобедим.',
        'О, сударыня, русская зависть… Она национальный двигатель, страной движет как паровая машина у британцев.',
        'Иногда нам не везёт в любви, и в таких случаях я начинаю пробежки. Во время бега организм теряет много жидкости, так что на слёзы ничего не остаётся.',
        'Каждый день мы проходим мимо большого числа людей. Людей, которых мы никогда не встретим больше. Или мимо тех, кто мог бы стать нашим другом.',
        'Слабак никогда не заговорит о прощении более сильного человека — для этого ему придётся самому стать сильным.',
        'Неправильно проявлять заботу о мире во всём мире, ежели в своём собственном доме мира нету.',
        'Любовь сама по себе банальна. Встреча, обещание, расставание, страдание… потом снова встреча. Людям надоедает это и они решают пожениться.',
        'Было бы здорово, если бы всем, кто нам нравится, мы тоже нравились. Это была бы совсем другая Вселенная.',
        'Нужно полюбить все вокруг, не обращать внимания на негатив. Быть счастливым — значит быть умным.',
        'Можно ли скучать по месту, в котором никогда не был, мечтать о девушке, которую никогда не видел?',
        'Когда сам себя уничтожаешь, кажется, будто виноват весь мир.',
        'Так что я не верю в дьявола, зла хватает и в самих людях.',
        'Война научила меня, что для того, чтобы зло восторжествовало, хорошим людям достаточно просто ничего не делать. Люди напуганы и предпочитают ни во что не вмешиваться.',
        'Блоги — это не литература. Это всё равно что граффити с запятыми.',
        'У нас нет завтра, поэтому нам наплевать на сегодня.',
        'У всех есть своя половинка, даже если вам нужна мотыга и компас, чтобы найти их.',
        'Почему мы не всегда узнаем, когда зарождается любовь, но всегда знаем, когда она кончается?',
        'Я не посмотрю, что ты пенсионер, так в морду съезжу, что сам себя с Алексеем путать будешь, понял?',
        'Если всегда продолжаешь заниматься тем, чем ты всегда занимался — навсегда остаёшься тем, кем и был. Ничего не изменится, пока ты сам не изменишь что-то…',
        'Женщинам правда не нужна, для них всегда важны только их чувства.',
        'Все великие художники одинаковы — жить с ними невозможно, но встречаться интересно.',
        'Иногда мы оказываемся на перекрестке и, какой бы путь ты ни выбрала, твоя жизнь навсегда изменится.',
        'И каллиграфия, и боевое искусство — оба они основываются на сочетании силы в руке и духа в сердце.',
        'Такова жизнь, мы рождаемся с заданными качествами. Нет смысла твердить «Я ничего не смогу». Придется импровизировать',
        'Тебе тошно, а ты улыбайся. Вот тогда будешь интересная женщина. Ходи, вроде тебя ни одна собака сроду не кусала. Голову кверху, грудь вперед, и улыбайся. Только не хохочи, это дурость.',
        'Какой кайф. Каждое утро теперь особенное, за это я благодарен. Жаль, в бутылки это чувство не разольешь. Как прекрасен каждый глоток жизни.',
        'Дурак! Когда в следующий раз вздумаешь угрожать пистолетом, проверь, заряжен ли он!',
        'Он был злой человек, но что еще хуже.. он пробудил зло во мне и мне с этим жить..',
        'Смотри. Смотри! Благодаря твоей картине, ЭТО не покроют морщины, не обезобразит старость. Ты мог представить, что человек сможет делать всё, что хочет, поддаваться соблазну, пока все вокруг будут восхищаться его красотой.',
        'Твоя битва за выживание начнется прямо сейчас. Не хочешь, чтобы тебя судили? Не будут. Считаешь, что ты недостаточно сильна? Ошибаешься. Боишься? Не бойся. У тебя есть все необходимое оружие. Сражайся.',
        'Понимаешь, кому-то суждено стать Нобелевским лауреатом, кому-то суждено стать балериной. Почему не нам?',
        'Возьмём, к примеру, опята. Они растут на пнях. Если придёшь в лес и тебе повезёт с пнём, то можно набрать целую гору… пней… ой, опят…',
        'Религия — это мрак для ума.',
        'Если тебе непременно надо мужа, выходи за глупого. Слишком уж хорошо знает умный, каких чудищ вы из них делаете.',
        'Никаких свадеб! Кто жил в браке, пусть остается в супружестве, все кроме одного! Остальные пусть воздержатся.',
        'Нужно делать невозможное. Нужно тяжело работать. И если у тебя получается, ты позитивен, у тебя появляется лучик надежды.',
        'Моя трагедия была в том, что я любил слова больше, чем любил женщин.',
    ],
    'код!'.decode('UTF-8'): [
        'Java - это, во многим, C++-',
        'Писать на С или С++ - то же самое, что пользоватся бензопилой без всех предохранителей',
        'В С++ сложнее выстрелить себе в ногу, но если у вас все-таки получается, то она оторвется полностью',
        'Дизайн машинного языка - это как прогулка по парку. Парку Юрского периода',
        'Среди языков программирования - не важно, как они структурированы - нет ни одного, который бы защитил программистов от написания плохих программ',
        'Самое лутшое в boolean - это то, что даже если ты неправ, ты теряешь всего один бит',
        'Требовать и эффективности, и гибкости от одной и той же программы - это все равно, что искать очаровательну и скромною жену... по-видимому, нам следует остановиться на чем-то одному из двух',
        'Пользователь не знает, что он хочет, пока не увидит то, что он получил',
        'Компютеры бесподобны: за несколько минут они могут совершить такую грандиозную ошибку, какую не в состоянии сделать множество людей за многие месацы',
        'Последние нововведения в C++ были созданы, чтобы исправить предедущие нововведения',
        'Машины должны работать. Люди должны думать.',
        'Руки чешутся поставить какой-нибуть дистрибутив Linux - к сессии|дедлайну',
        'Глупец, лишенний способности посмеяться на собой вмести с другими, не сможет долго выносить программирования',
        'Лучше написать и выполнить неполные тесты, чем не выполнить полные тесты',
        'Любой дурак может написать код, понятный компьютеру. Хороший программист пишет код, понятный человеку',
        'Отладка кода вдвое сложнее, чем его написание. Так что если вы пишете код, настолько умно, насколько можете, то вы по определению недостаточно сообразительны, чтобы его отлаживать',
        'Сначала учите науку программирования и всю теорию. Далее выработайте свой программисткий стиль. Затем забудьте все и просто программируйте',
        'Я не считаю себя замечательным программистом. Я просто хороший программист с замечательными привычками',
        'Ходить по воде и разрабатывать программы, следуя спецификации, очень просто... если они заморожены',
        'Днем за днем я проживаю свою жизнь по одний строчке безошибочного кода',
        'Есть два способа создания дизайна программы. Один из них, это сделать его настолько простым, что в нем, очевидно, не будет недостатков. Другой способ - сделать его настолько запутанным, что в нем не будет очевидных недостатков.'
    ],
    'сеть!'.decode('UTF-8'): [
        'Я знаю отличную шутку про UDP, но не факт, что она до вас дойдет.',
        'Я знаю отличную шутку про TCP, но если она до вас не дойдет, то я повторю.',
        'А кто знает отличную шутку про ARP?',
        'А вы слышали шутку про ICMP?',
        'Вам еще кто-то рассказывал шутку про STP?',
        'Я подожду Антона и расскажу классную шутку про QoS.',
        'Про MTU тоже есть кла',
        '<шутка><смешная/><про>XML',
        'А про FSMO роли шутить могут не более пяти человек.',
        'Подождите все, я расскажу шутку о сети типа "шина".',
        'Я бы рассказал отличную шутку про Token Ring, но сейчас не моя очередь.',
        'Стой-стой, послушай сначала шутку о прерываниях.',
        'Помню времена, когда шутка про модем пшшшшшшш.....',
        'Только что, специально для сообщества пришла шутка про мультикаст.',
        'Жаль, что шутка про Fault Tolerance не может состоять больше, чем из одного слова.',
        'Настало время рассказать шутку про NTP.',
        'Я сейчас расскажу отличную шутку про VPN, но ее поймет только один.',
        'К шутке про SCTP вначале должны все подготовиться.',
        'Из-за одного, кто зевнул, придётся заново рассказывать шутку про frame relay в топологии point-to-multipoint.',
        'А шутки про HDLC обычно не понимают те, кто знает другие шутки про HDLC.',
        'Про DWDM шутят сразу несколькими голосами.',
        'Шутка про Е3 - это 30 одинаковых шуток про Е1 и еще две шутки, понятных только тем, кто в теме.'
    ]
}








